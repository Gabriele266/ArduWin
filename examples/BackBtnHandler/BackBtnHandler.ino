 ...
 * Esempio 06: BackBtnHandler
 * In questo esempio creeremo una finestra e, su richiesta dell' utente tramite la comunicazione seriale 
 * emuleremo la pressione del pulsante indietro.
...
GWindow *newWin = new GWindow("new", "Esempio 07");
...
void handler(GEvent *event){
  Serial.println("Premuto il pulsante indietro");
}
...
void setup() {
  ...
  Serial.begin(57600);
  // Scrivo l'esempio
  Serial.println("Esempio 07 BackBtnHandler");
  ...
  newWin->setBackBtnHandle(handler);
...
void loop() {
  ...
  if(Serial.available() > 0){
    ...
    if(v == 'c'){
      ...
      newWin->updateControls(createLocation(1,0));
    }
  }
